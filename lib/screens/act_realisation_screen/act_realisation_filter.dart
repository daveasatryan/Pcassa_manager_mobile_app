...
class ActRealisationFilter extends StatefulWidget {
  ActRealisationFilter({Key? key}) : super(key: key);
  ...
  @override
  State<ActRealisationFilter> createState() => _ActRealisationFilterState();
}
...
class _ActRealisationFilterState extends State<ActRealisationFilter> {
  // bool? statusChecked;
                                ...
                                children: [
                                  Expanded(
                                    child: CreateAndEditForm(
                                      onTap: () {
                                        setState(() {
                                          ActRealisationFilter
                                              .firstDatecontraller.text = "";
                                        });
                                        showFirstDate();
                                      },
                                      textInputType: TextInputType.number,
                                      contraller: ActRealisationFilter
                                          .firstDatecontraller,
                                      fildName: MiddleText(
                                        text: 'firstDate'.tr(),
                                      ),
  ...
  searchDocument() async {
    ...
    await actRealisationState.getActRealisation(
        sklname:
            ActRealisationFilter.firstWarehouseContraller == "notSelected" ||
                    ActRealisationFilter.firstWarehouseContraller == null
                ? ""
                : ActRealisationFilter.firstWarehouseContraller,
        data_s: ActRealisationFilter.firstDatecontraller.text == null
            ? ""
            : ActRealisationFilter.firstDatecontraller.text,
        data_e: ActRealisationFilter.secondDatecontraller.text == null
            ? ""
            : ActRealisationFilter.secondDatecontraller.text,
        ...
        name: ActRealisationFilter.productItemContraller == null
            ? ""
            : ActRealisationFilter.productItemContraller.toString(),
        order_by: "");
   setState(() {
  ...
  showFirstDate() async {
    ActRealisationFilter.pickedDate1 = await showDatePicker(
    ...
    if (ActRealisationFilter.pickedDate1 != null) {
      String formattedDate = DateFormat('dd.MM.yyyy').format(ActRealisationFilter
          .pickedDate1!); // format date in required form here we use yyyy-MM-dd that means time is removed

      setState(() {
        if (ActRealisationFilter.pickedDate2 != null) {
          if (ActRealisationFilter.pickedDate1!
              .isAfter(ActRealisationFilter.pickedDate2!)) {
            showDialog<bool?>(
              context: context,
            ...
            setState(() {
              ActRealisationFilter.firstDatecontraller.text = formattedDate;
            });
          }
        } else {
          setState(() {
  ...
  showSecondDate() async {
    ActRealisationFilter.pickedDate2 = await showDatePicker(
      initialEntryMode: DatePickerEntryMode.calendarOnly,
      context: context,
      initialDate: DateTime.now(), //get today's date
      firstDate: DateTime(
    ...
    if (ActRealisationFilter.pickedDate2 != null) {
      ...
      setState(() {
        if (ActRealisationFilter.pickedDate1 != null) {
          if (ActRealisationFilter.pickedDate2!
              .isBefore(ActRealisationFilter.pickedDate1!)) {
            showDialog<bool?>(
              context: context,
              barrierDismissible: false,
              builder: (BuildContext context) {
            ...
            setState(() {
              ActRealisationFilter.secondDatecontraller.text = formattedDate;
            });
          }
        } else {
          setState(() {
